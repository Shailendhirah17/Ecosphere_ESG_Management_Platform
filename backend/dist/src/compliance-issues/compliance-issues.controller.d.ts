import { ComplianceIssuesService } from './compliance-issues.service';
import { CreateComplianceIssueDto } from './dto/create-compliance-issue.dto';
import { UpdateComplianceIssueDto } from './dto/update-compliance-issue.dto';
export declare class ComplianceIssuesController {
    private readonly complianceIssuesService;
    constructor(complianceIssuesService: ComplianceIssuesService);
    create(createComplianceIssueDto: CreateComplianceIssueDto): string;
    findAll(): import("@prisma/client").Prisma.PrismaPromise<{
        id: string;
        status: string;
        description: string;
        audit_id: string | null;
        severity: string;
        owner_employee_id: string;
        due_date: Date;
        raised_date: Date;
    }[]>;
    findOne(id: string): string;
    update(id: string, updateComplianceIssueDto: UpdateComplianceIssueDto): string;
    remove(id: string): string;
}
